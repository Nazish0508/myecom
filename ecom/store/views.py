...
def search(request):
    #determine is they filled form
    if request.method == "POST":
        searched = request.POST['searched']
        #query the products db model
        searched = Product.objects.filter(Q(name__icontains=searched) | Q(decsription__icontains=searched))
        #test for null
        if not searched:
...
def login_user(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            if saved_cart:
            #convert from str to dictionary usinf json
